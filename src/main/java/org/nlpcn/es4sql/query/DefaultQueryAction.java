...
public class DefaultQueryAction extends QueryAction {
    ...
    public DefaultQueryAction(Client client,
                              Select select) {
        super(client,
              select);
        this.select = select;
    ...
    public SqlElasticSearchRequestBuilder explain() throws SqlParseException {
        ...
        updateRequestWithIndexAndRoutingOptions(select,
                                                request);
        updateRequestWithHighlight(select,
                                   request);
    ...
    private void handleScriptField(MethodField method) throws SqlParseException {
        ...
        if (params.size() == 2) {
            request.addScriptField(params.get(0).value.toString(),
                                   new Script(params.get(1).value.toString()));
     ...
     */
    private void setLimit(int from,
                          int size) {
